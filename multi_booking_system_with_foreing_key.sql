...
CREATE TABLE Room_Types (
    id INT PRIMARY KEY AUTO_INCREMENT,
    hotel_id INT,
    room_type_name VARCHAR(100) NOT NULL, -- e.g., Single, Double, Suite
...
-- Booking_Hotels table (connects Bookings table with Hotels)
...
CREATE TABLE Car_Types (
    id INT PRIMARY KEY AUTO_INCREMENT,
    type_name VARCHAR(100) NOT NULL,  -- e.g., Sedan, SUV, Van
    capacity INT
);
...
CREATE TABLE Cars (
    ...
    FOREIGN KEY (provider_id) REFERENCES Car_Providers(id),
    FOREIGN KEY (car_type_id) REFERENCES Car_Types(id)
);

-- Booking_Cars table (connects Bookings table with Cars)
...
-- Drivers table (optional if car with driver)
